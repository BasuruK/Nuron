...
- **GitHub outage observed 2026-08-17:** GraphQL returned 5xx (notably 503) during that incident. If you observe relevant 5xx failures on GraphQL or other GitHub API writes, fall back to `gh api` REST endpoints; only then apply write retries (8–10 max) and post-write verification. After a lost or ambiguous POST (timeout, 502/503, empty body), **reconcile before retrying**: search for a deterministic marker (title, `NU-00N` prefix, or a unique comment token) and reuse the existing issue or comment if the write landed; for sub-issues and dependencies, GET the relationship first and skip the POST if it already exists. Retry only confirmed transient failures (5xx / timeout with no matching resource). Never blindly retry a create after a lost response. Absent those 5xx failures, REST fallback, write retries, and post-write verification are not required.
